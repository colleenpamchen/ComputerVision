...
imagesc(C); 
colorbar 
title(sprintf('Class confusion matrix \n nearest neighbor average classification accuracy: %.3f \n using Euclidean distance',accuracy))
...
for k=1:length(knn) 
    ...
    imagesc(C); 
    colorbar 
    title( sprintf( 'Classification error: %f at KNN=%d \n Euclidean distance',error, knn(k) ) )
...
for k=1:length(knn) 
    ...
    imagesc(C); 
    colorbar 
    title( sprintf( 'Classification accuracy: %f at KNN=%d \n Normalized correlation',accuracy, knn(k) ) )
...
%     label
    colorbar 
    title( sprintf( 'Classification accuracy: %f at KNN=%d \n Hamming distance',accuracy, knn(k) ) )
