...
imageDataRowRedEn(:,1) = imageDataRowRedEn(:,1)*1000;
...
newImageDataRowRedEn(1,:) = newImageDataRowRedEn(1,:)./1000;
newImageDataRedEn = reshape(newImageDataRowRedEn,[numRow numColumn 3]);

imageTitle = strcat(num2str(k),'-means_',imname);
imwrite(newImageData,imageTitle,'JPG');

imageTitleRedEn = strcat(num2str(k),'-means_redEn_',imname);
imwrite(newImageDataRedEn,imageTitleRedEn,'JPG');

figure; imshow(newImageDataRedEn)
...
% imageTitleRedEn = strcat(num2str(k),'-means_redEn_',imname);
% imwrite(newImageDataRedEn,imageTitleRedEn,'PNG');
